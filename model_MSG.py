...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        nn.init.trunc_normal_(self.rel_pos_msg, std=.02)
...
class MSGTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        nn.init.trunc_normal_(self.msg_tokens, std=.02)
...
def msg_tiny_p4_win7_224(pretrained=False, **kwargs):
    drop_rate = kwargs.pop('drop_path_rate', 0.1)
    model = MSGTransformer(embed_dim=64,
                           depths=[2, 4, 12, 4],
                           num_heads=[2, 4, 8, 16],
                           window_size=7,
                           shuffle_size=[4, 4, 2, 1],
                           manip_type='shuf',
...
def msg_small_p4_win7_224(pretrained=False, **kwargs):
    drop_rate = kwargs.pop('drop_path_rate', 0.3)
    model = MSGTransformer(embed_dim=96,
                           depths=[2, 4, 12, 4],
                           num_heads=[3, 6, 12, 24],
                           window_size=7,
                           shuffle_size=[4, 4, 2, 1],
...
def msg_base_p4_win7_224(pretrained=False, **kwargs):
    drop_rate = kwargs.pop('drop_path_rate', 0.5)
    model = MSGTransformer(embed_dim=96,
                           depths=[2, 4, 28, 4],
                           num_heads=[3, 6, 12, 24],
                           window_size=7,
